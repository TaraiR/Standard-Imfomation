...
<ul>
  ...
  <li><strong>辞書攻撃</strong>: よく使われるパスワード辞書を使って効率よく試す</li>
      ...
      },
      {
        ...
        content: `
<h3>暗号化の目的</h3>
<p>データを第三者が読めない形式に変換し、<strong>機密性</strong>を保護することです。</p>

<h3>共通鍵暗号方式（対称鍵暗号）</h3>
<p>暗号化と復号に<strong>同じ鍵</strong>を使う方式です。</p>
<ul>
  <li>高速で大量データの暗号化に向く</li>
  <li>鍵の安全な配送が課題（<strong>鍵配送問題</strong>）</li>
  <li>代表例: <strong>AES</strong>（現在の標準）、DES（古い規格、現在は非推奨）</li>
</ul>

<h3>公開鍵暗号方式（非対称鍵暗号）</h3>
<p>暗号化と復号に<strong>異なる鍵ペア</strong>（公開鍵・秘密鍵）を使う方式です。</p>
<ul>
  <li>公開鍵は誰でも入手可能。秘密鍵は本人だけが保持。</li>
  <li>鍵配送問題を解決できる</li>
  <li>共通鍵方式より遅い。少量データや鍵交換に使う。</li>
  <li>代表例: <strong>RSA</strong>、楕円曲線暗号（ECC）</li>
</ul>
<p><strong>暗号化</strong>: 受信者の<strong>公開鍵</strong>で暗号化 → 受信者の<strong>秘密鍵</strong>で復号</p>

<h3>ハイブリッド暗号方式</h3>
<p>TLSなどで使われる実用的な方式です。</p>
<ol>
  <li>公開鍵暗号で<strong>共通鍵を安全に交換</strong>する</li>
  <li>交換した共通鍵で<strong>実際のデータを高速暗号化</strong>する</li>
</ol>

<h3>ハッシュ関数</h3>
<p>任意のデータから固定長の<strong>ハッシュ値（ダイジェスト）</strong>を生成します。一方向性があり、ハッシュ値から元データを復元できません。</p>
<ul>
  <li>パスワードの保存（平文ではなくハッシュを保存）</li>
  <li>データ改ざん検知（ダウンロードしたファイルの検証）</li>
  <li>代表例: <strong>SHA-256</strong>（現在の標準）、MD5（脆弱性あり非推奨）</li>
</ul>
        `,
        diagram: 'crypto',
        questions: [
